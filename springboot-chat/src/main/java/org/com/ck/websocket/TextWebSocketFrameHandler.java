...
public class TextWebSocketFrameHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
	...
	protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) throws Exception {
		...
		if ("100001".equals(obj.getMsgType())) {
			sessionChannelMap.put(obj.getFrom(), incoming);
		} else if ("100002".equals(obj.getMsgType())) {
			incoming.writeAndFlush(new TextWebSocketFrame(obj.getFrom() + ":" + obj.getMsg()));

			sessionChannelMap.get(obj.getTo()).writeAndFlush(new TextWebSocketFrame(obj.getFrom() + ":" + obj.getMsg()));
		} else {
